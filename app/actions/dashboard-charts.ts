...
/**
 * Get weekly summary from Tinybird
 */
async function getWeeklySummaryFromTinybird(currentUser: User) {
  const tinybird = getTinybirdClient()
...
/**
 * Get weekly summary from MongoDB (fallback)
 */
async function getWeeklySummaryFromMongo(currentUser: User) {
  const { db } = await connectToDatabase()
...
export async function getWeeklySummary() {
  try {
    ...
    if (isTinybirdReadsEnabled() && isTinybirdConfigured()) {
      try {
        console.log('[Tinybird] Fetching weekly summary from Tinybird')
        return await getWeeklySummaryFromTinybird(currentUser)
      } catch (error) {
        console.error('[Tinybird] Weekly summary query failed, falling back to MongoDB:', error)
      }
    }

    // Fallback to MongoDB
    return getWeeklySummaryFromMongo(currentUser)
  } catch (error) {
