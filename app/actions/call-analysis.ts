...
export async function updateCallAnalysisNoShowStatus(
  callAnalysisId: string,
  noShow: boolean,
  raisonNoShow?: string
): Promise<{ success: boolean; message: string }> {
  try {
    console.log('Updating call analysis no-show status:', callAnalysisId, noShow, raisonNoShow)
    ...
    const updateResult = await db.collection<CallAnalysis>(COLLECTIONS.CALL_ANALYSIS)
      .updateOne(
        { _id: new ObjectId(callAnalysisId) },
        {
          $set: {
            no_show: noShow,
            raison_no_show: noShow ? (raisonNoShow || 'Prospect absent') : '',
            updatedAt: new Date()
          }
        }
      )
    ...
    console.log('Call analysis no-show status updated successfully:', callAnalysisId)
    ...
    return { success: true, message: noShow ? 'Marqué comme no-show' : 'No-show retiré' }
  } catch (error) {
    console.error('Error updating call analysis no-show status:', error)
    return { success: false, message: 'Erreur lors de la mise à jour' }
  }
}
...
export async function updateDealValue(
  ...
  try {
    console.log('Updating call analysis deal value:', callAnalysisId, dealValue)
    ...
    return { success: true, message: 'Montant du deal mis à jour avec succès' }
  } catch (error) {
    console.error('Error updating call analysis deal value:', error)
    return { success: false, message: 'Erreur lors de la mise à jour' }
  }
}
