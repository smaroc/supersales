...
export function DashboardNav({ isMobileOpen = false, onMobileClose }: DashboardNavProps) {
  ...
  const NavContent = ({ mobile = false }: { mobile?: boolean }) => (
    <div className="flex h-full flex-col">
      {/* Quick Actions Search & Toggle - Desktop only */}
      {!mobile && (
        <div className={cn(
      ...
      <div className={cn(
        ...
        mobile ? "px-3" : (isCollapsed ? "px-2" : "px-3")
      )}>
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      ...
                      mobile ? 'gap-3 px-3 py-3' : (isCollapsed ? 'justify-center p-2' : 'gap-2.5 px-3 py-2'),
                      isActive
                  ...
                  >
                    <item.icon className={cn("flex-shrink-0", mobile ? "h-5 w-5" : "h-[15px] w-[15px]")} strokeWidth={isActive ? 2.5 : 2} />
                    {(mobile || !isCollapsed) && <span className={cn(mobile ? "text-sm" : "text-[11px]", "leading-tight")}>{item.name}</span>}
                  </Link>
                )
              })}
            </div>
      ...
      <div className={cn(
        ...
        <div className="space-y-0.5">
          <ThemeToggle collapsed={!mobile && isCollapsed} />
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <nav className={cn(
        "hidden md:block h-[calc(100vh-72px)] shrink-0 self-start bg-white dark:bg-black border-r border-gray-200/80 dark:border-zinc-800 transition-all duration-300 relative",
      ...
      {isMobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          />
          {/* Drawer */}
          <nav className="fixed top-0 left-0 h-full w-[280px] bg-white dark:bg-black shadow-2xl animate-in slide-in-from-left duration-300">
